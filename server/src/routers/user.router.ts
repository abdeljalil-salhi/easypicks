...
userRouter.post(
  ...
  expressAsyncHandler(async (req: express.Request, res: express.Response) => {
    const user: TUser = await User.findOne({ email: req.body.email });
    if (user) {
...
userRouter.get(
  ...
  isAuth,
  expressAsyncHandler(async (req: express.Request, res: express.Response) => {
    const user: TUser = await User.findById(req.params.id);
...
userRouter.put(
  ...
  expressAsyncHandler(
    async (req: express.Request & { user: TUser }, res: express.Response) => {
      const user: TUser = await User.findById(req.user._id);
      if (user) {
        user.name = req.body.name || user.name;
        user.email = req.body.email || user.email;
